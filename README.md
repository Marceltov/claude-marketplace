...
| `ship-pr` | Branch, commit, PR, an ADR when warranted, and a merge that waits for explicit confirmation; nudges you when work is left unshipped; `/issues`, `/actions`, `/prs`, `/pr`, `/repo` open GitHub/GitLab views. | [this repo](plugins/ship-pr) |
